import { AuthNonceResponse } from "./AuthService";
import Logger from "../../logger";
import { IJWTHandler, IAuthenticationHandler } from '../../client';
...
export class JwtAuthenticationHandler extends BaseAuthenticationHandler implements IAuthenticationHandler {
    ...
    protected async login(): Promise<LoginDetails> {
        ...
        const token = await this.tokenHandler.readToken();
        if(token) {
            try {
                const r = await this.authService.verify({
                    token
                });
                if(r.valid) {
                    return {
